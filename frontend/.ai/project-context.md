...
`Task` → `/tasks/{id}`, `Event` → `/events/{id}`, `Duty` → без цел засега (модул 5 е отложен,
няма екран за лична справка на дежурство, към който да води).
...
Пропуснат модул 5 (дежурства) — отбелязан в roadmap.md като отворен въпрос: дежурствата
явно се водят от отделна външна система, връзката с този backend е неизяснена.
